...
local rhelimgbuildjob = imgbuildjob {
  ...
  is_arm:: std.length(std.findSubstr('-arm64', tl.image)) > 0,
  is_byos:: std.length(std.findSubstr('-byos', tl.image)) > 0,
  is_eus:: std.length(std.findSubstr('-eus', tl.image)) > 0,
  is_lvm:: std.length(std.findSubstr('-lvm', tl.image)) > 0,
  is_sap:: std.length(std.findSubstr('-sap', tl.image)) > 0,
